...
class GpioLcd(LcdApi):
    ...
    def hal_pulse_enable(self):
        print("""Pulse the enable line high, and then low again.""")
        self.enable_pin.low()
        utime.sleep_us(1)
        self.enable_pin.high()
        utime.sleep_us(1)       # Enable pulse needs to be > 450 nsec
        self.enable_pin.low()
        utime.sleep_us(100)     # Commands need > 37us to settle

    def hal_write_init_nibble(self, nibble):
        print("""Writes an initialization nibble to the LCD.
        This particular function is only used during initialization.
        """)
        self.hal_write_4bits(nibble >> 4)

    def hal_backlight_on(self):
        print("""Allows the hal layer to turn the backlight on.""")
        if self.backlight_pin:
            self.backlight_pin.high()

    def hal_backlight_off(self):
        print("""Allows the hal layer to turn the backlight off.""")
        if self.backlight_pin:
            self.backlight_pin.low()

    def hal_write_command(self, cmd):
        print("""Writes a command to the LCD.
    ...
    def hal_write_data(self, data):
        print("""Write data to the LCD.""")
        self.rs_pin.high()
        self.hal_write_8bits(data)

    def hal_write_8bits(self, value):
        print("""Writes 8 bits of data to the LCD.""")
        if self.rw_pin:
            self.rw_pin.low()
        if self._4bit:
    ...
    def hal_write_4bits(self, nibble):
        print("""Writes 4 bits of data to the LCD.""")
